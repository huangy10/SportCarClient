...
//            make.edges.equalTo(toastContainer)
    ...
    func showConfirmToast(_ title: String, message: String, target: AnyObject, confirmSelector: Selector, cancelSelector: Selector, onSelf: Bool = true) -> UIView {
        ...
        let container = superview.addSubview(UIView.self).config(UIColor.clear)
            .layout { (make) in
                make.edges.equalTo(superview)
        }
        let bgView = container.addSubview(UIView.self).config(UIColor(white: 0, alpha: 0.2))
            .layout { (make) in
                make.edges.equalTo(superview)
        }
        bgView.layer.opacity = 0
        
        let toast = container.addSubview(UIView.self).config(UIColor.white).toRound(4).layout { (make) in
            make.centerX.equalTo(superview)
            make.top.equalTo(superview.snp.bottom).offset(30)
            make.width.equalTo(250)
            make.height.equalTo(150)
        }.addShadow()
        let titleLbl = toast.addSubview(UILabel.self)
        ...
        let confirmBtn = toast.addSubview(UIButton.self)
...
//        
    ...
    func configureBlurredBackground() {
        ...
        bg = view.addSubview(UIView.self).config(UIColor.clear)
            .layout({ (make) in
        ...
        bg.addSubview(UIView.self).config(UIColor(white: 0, alpha: 0.3))
        bg.layer.opacity = 0
    }
    ...
    func configureDialog() {
        let height = desLblHeight + 160 - 44
        dialog = view.addSubview(UIView.self)
...
//                make.bottom.equalTo(view.snp.centerY)
    ...
    func configureTitleLbl() {
        titleLbl = dialog.addSubview(UILabel.self)
    ...
    func configureDesLbl() {
        desLbl = dialog.addSubview(UILabel.self)
            .layout({ (make) in
                make.left.equalTo(titleLbl)
    ...
    func configureConfirmBtn() {
        ...
        confirmBtn = dialog.addSubview(UIButton.self)
    ...
    func configureCancelBtn() {
        ...
        cancelBtn = dialog.addSubview(UIButton.self)
