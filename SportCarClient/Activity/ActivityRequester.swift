...
class ActivityRequester: BasicRequester {
    ...
    func getNearByActivities(userLocation: CLLocation, queryDistance: Double, skip: Int, limit: Int, onSuccess: (JSON?)->(), onError: (code: String?)->()) -> Request {
        let lat: Double = userLocation.coordinate.latitude
        let lon: Double = userLocation.coordinate.longitude
        return get(
            urlForName("nearby"),
            parameters: ["lon": lon, "lat": lat, "query_distance": queryDistance, "limit": limit, "skip": skip],
            responseDataField: "acts",
            onSuccess: onSuccess, onError: onError
        )
    }
